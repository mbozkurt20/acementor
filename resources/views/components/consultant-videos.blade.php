<section id="facts" class="video-counter pt-70">
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-lg-10">
                <div class="section-title text-center pb-40">
                    <div class="line m-auto"></div>
                    <h3 class="title">Consultant <span> Videos</span></h3>
                </div> <!-- section title -->
            </div>
        </div> <!-- row -->
        <div class="row mb-4 p-1">
            <div class="col-lg-4">
                <div class="video-content wow fadeIn" data-wow-duration="1s" data-wow-delay="0.5s">
                    <div class="video-wrapper">
                        <img height="200" src="https://i.stack.imgur.com/HUDoZ.png" alt="video">
                        <div class="video-icon">
                            <a href="https://www.youtube.com/watch?v=r44RKWyfcFw" class="video-popup"><i class="lni-play"></i></a>
                        </div>
                    </div> <!-- video wrapper -->
                </div> <!-- video content -->
            </div>
            <div class="col-lg-8">
                <div class="counter-wrapper wow fadeIn" data-wow-duration="1s" data-wow-delay="0.8s">
                    <div class="counter-content">
                        <div class="section-title">
                            <div class="line"></div>
                            <h3 class="title"> facts <span> this about app</span></h3>
                        </div> <!-- section title -->
                        <p class="text">Lorem ipsum dolor sit amet, consetetur sadipscing elitr, seiam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua.</p>
                    </div> <!-- counter content -->
                </div> <!-- counter wrapper -->
            </div>
        </div> <!-- row -->

        <div class="row mb-4 p-1">
            <div class="col-lg-4">
                <div class="video-content wow fadeIn" data-wow-duration="1s" data-wow-delay="0.5s">
                    <div class="video-wrapper">
                        <img height="200" src="https://i.stack.imgur.com/HUDoZ.png" alt="video">
                        <div class="video-icon">
                            <a href="https://www.youtube.com/watch?v=r44RKWyfcFw" class="video-popup"><i class="lni-play"></i></a>
                        </div>
                    </div> <!-- video wrapper -->
                </div> <!-- video content -->
            </div>
            <div class="col-lg-8">
                <div class="counter-wrapper wow fadeIn" data-wow-duration="1s" data-wow-delay="0.8s">
                    <div class="counter-content">
                        <div class="section-title">
                            <div class="line"></div>
                            <h3 class="title"> facts <span> this about app</span></h3>
                        </div> <!-- section title -->
                        <p class="text">Lorem ipsum dolor sit amet, consetetur sadipscing elitr, seiam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua.</p>
                    </div> <!-- counter content -->
                </div> <!-- counter wrapper -->
            </div>
        </div> <!-- row -->

        <div class="collapse" id="collapseExample">
            <div class="row mb-4 p-1">
                <div class="col-lg-4">
                    <div class="video-content wow fadeIn" data-wow-duration="1s" data-wow-delay="0.5s">
                        <div class="video-wrapper">
                            <img height="200" src="https://i.stack.imgur.com/HUDoZ.png" alt="video">
                            <div class="video-icon">
                                <a href="https://www.youtube.com/watch?v=r44RKWyfcFw" class="video-popup"><i class="lni-play"></i></a>
                            </div>
                        </div> <!-- video wrapper -->
                    </div> <!-- video content -->
                </div>
                <div class="col-lg-8">
                    <div class="counter-wrapper wow fadeIn" data-wow-duration="1s" data-wow-delay="0.8s">
                        <div class="counter-content">
                            <div class="section-title">
                                <div class="line"></div>
                                <h3 class="title"> facts <span> this about app</span></h3>
                            </div> <!-- section title -->
                            <p class="text">Lorem ipsum dolor sit amet, consetetur sadipscing elitr, seiam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua.</p>
                        </div> <!-- counter content -->
                    </div> <!-- counter wrapper -->
                </div>
            </div> <!-- row -->
            <div class="row mb-4 p-1">
                <div class="col-lg-4">
                    <div class="video-content wow fadeIn" data-wow-duration="1s" data-wow-delay="0.5s">
                        <div class="video-wrapper">
                            <img height="200" src="https://i.stack.imgur.com/HUDoZ.png" alt="video">
                            <div class="video-icon">
                                <a href="https://www.youtube.com/watch?v=r44RKWyfcFw" class="video-popup"><i class="lni-play"></i></a>
                            </div>
                        </div> <!-- video wrapper -->
                    </div> <!-- video content -->
                </div>
                <div class="col-lg-8">
                    <div class="counter-wrapper wow fadeIn" data-wow-duration="1s" data-wow-delay="0.8s">
                        <div class="counter-content">
                            <div class="section-title">
                                <div class="line"></div>
                                <h3 class="title"> facts <span> this about app</span></h3>
                            </div> <!-- section title -->
                            <p class="text">Lorem ipsum dolor sit amet, consetetur sadipscing elitr, seiam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua.</p>
                        </div> <!-- counter content -->
                    </div> <!-- counter wrapper -->
                </div>
            </div> <!-- row -->
        </div>
        <div class="align-items-center">
            <a class="btn-hover-white w-25 text-center" data-toggle="collapse" href="#collapseExample" role="button" aria-expanded="false" aria-controls="collapseExample">
                Daha Fazlası
            </a>
        </div>

    </div> <!-- container -->

    <style>
        .video-content {
            perspective: 1000px;
            transition: transform 0.5s;
        }

        .video-wrapper {
            position: relative;
            transform-style: preserve-3d;
            transition: transform 0.5s;
        }

        .video-content:hover .video-wrapper {
            transform: rotateY(20deg);
        }

        /* Add any additional styling as needed */

    </style>
</section>


